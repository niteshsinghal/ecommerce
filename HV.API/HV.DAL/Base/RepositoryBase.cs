...
namespace HV.DAL
{
    public class RepositoryBase : IBaseRepository
    {
        ...
        protected RepositoryBase(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            _httpContextAccessor = httpContextAccessor;
            _configuration = configuration;

            string connectionString = Utility.ConfigurnConnectionString(_configuration["ConnectionStrings:MasterDatabase"], _httpContextAccessor.HttpContext.User.Identity.GetClientDatabase());
        ...
        public TScalar ExecuteScalar<TScalar>(string query, object param = null, CommandType commandType = CommandType.StoredProcedure)
        {
            try
            {
                return _connection.ExecuteScalar<TScalar>(query, param, commandType: commandType, commandTimeout: _timeout);
            }
            finally
            {
                this.Dispose();
